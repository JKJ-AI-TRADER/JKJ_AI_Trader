from modules.nifty_option_opportunity_historical_validation import (
    validate_historical_opportunity,
)
...
def test_insufficient_observations():
    ...
    result = validate_historical_opportunity(observations)

    assert result["Status"] == "INSUFFICIENT_OBSERVATIONS"


def test_supported_strong_opportunity():
    observations = [
        build_observation("10:00:00", 100, 23000, 1000, 500),
...
def test_conflicting_opportunity():
    observations = [
...
def test_invalid_input():
    result = validate_historical_opportunity(None)
...
if __name__ == "__main__":
    ...
    test_supported_strong_opportunity()
    test_conflicting_opportunity()
    test_invalid_observation()
    test_invalid_input()

    print("V12.8 Stage 4 Historical Opportunity Validation: PASS")